...
(ipp_releases_original['DOS'].isna()).sum() #0 
(ipp_releases_original['FILE_REFERENCE'].isna()).sum()
ipp_releases_original[ipp_releases_original['FILE_REFERENCE'].isna()]

ipp_releases_original.loc[ipp_releases_original['FILE_REFERENCE'].isna(),'FILE_REFERENCE'] = ipp_releases_original['NOMS_ID']
ipp_releases_original[ipp_releases_original['NOMS_ID'] == 'A2464AA']

ipp_releases_original['FIRST_DOS'] = ipp_releases_original.groupby('FILE_REFERENCE')['DOS'].transform(lambda x: x.min())

ipp_releases_original[ipp_releases_original['RELEASE_DATE'] <= ipp_releases_original['FIRST_DOS']]
...
releases[(releases['FIRST_REL_DATE'] <= releases['FIRST_DOS'])  | (releases['FIRST_REL_DATE'].isna())] # none
...
releases['YEARS_SINCE_FIRST_REL'] = releases.apply(lambda x: TimeDiffs.year_diff(x['FIRST_REL_DATE'],x['RELEASE_DATE']),axis=1)

releases['YEARS_SINCE_FIRST_REL'].value_counts(dropna=False).sort_index()

less_3 = releases[releases['YEARS_SINCE_FIRST_REL'] < 3].copy()
less_3 = less_3.drop_duplicates('FILE_REFERENCE',keep='first')
less_3['YEARS_SINCE_FIRST_REL'].value_counts(dropna=False).sort_index()

less_3.resample('Y',on = 'RELEASE_DATE')['YEARS_SINCE_FIRST_REL'].size()
