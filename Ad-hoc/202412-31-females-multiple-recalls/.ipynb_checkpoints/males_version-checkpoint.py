...
recalls['RECALL_LENGTH'] = np.where(
    recalls['RECALL_TYPE_DESCRIPTION'].astype(str).str.contains("fix|ftr", case=False, na=False),
    "Fixed Term",
    "Standard"
)

recalls.pivot_table(index=['RECALL_LENGTH','RECALL_TYPE_DESCRIPTION'],aggfunc='size')

recalls = recalls[recalls['RECALL_LENGTH']=='Fixed Term']

# Determine recall number for each combination of date of sentence and licence revocation date

recalls['DOS'].isna().sum() # 10 cases with missing date of sentece, good
recalls['LICENCE_REVOKE_DATE'].isna().sum() # 0
recalls['NOMS_ID'].isna().sum() # 0
recalls['FILE_REFERENCE'].isna().sum() # 0
recalls['PRISON_NUMBER'].isna().sum() # 0

# check duplicates

recalls.duplicated(['PRISON_NUMBER','DOS','LICENCE_REVOKE_DATE'],keep='first').sum() # 1
recalls.duplicated(['PRISON_NUMBER','LICENCE_REVOKE_DATE'],keep='first').sum() # 1
...
recalls_2.to_excel("female_multiple_recalls_last_12_mths.xlsx",engine='xlsxwriter')