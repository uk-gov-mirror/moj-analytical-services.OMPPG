...
pop[pop.duplicated(['FILE_REFERENCE','DATE_OF_HOSPITAL_ORDER'], keep = False)] # 6 cases, but authority for dentention differs
...
pop_off[pop_off['COURT_OFFENCE_TEXT'].str.contains('drug|theft|suspended|prevent|shpo|sopo', case=False, na=False)]['COURT_OFFENCE_TEXT'].value_counts(dropna=False)

pop_off = pop_off[~pop_off['COURT_OFFENCE_TEXT'].str.contains('drug|theft|suspended|prevent|shpo|sopo', case=False, na=False)]
len(pop_off) # 438

pop_off.head()

pop_off.to_excel("output/pop_inc_transfers.xlsx")
