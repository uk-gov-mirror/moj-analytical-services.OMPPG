...
pop = pd.read_csv(f's3://alpha-omppg/isp-population/raw/DAO_2025_03_31.csv')

pop.columns = pop.columns.str.upper()
pop.head()
pop.columns

pop['SENTENCESTATUSCUSTOM'].value_counts(dropna=False)
pop['SENTENCESTATUS'].value_counts(dropna=False)
pop['SENTENCE_LENGTH_BANDED'].value_counts(dropna=False)

recall = pop[pop['SENTENCESTATUS']=='(7) Recall']

recall['SENTENCESTATUSCUSTOM'].value_counts(dropna=False)
recall['SENTENCE_LENGTH_BANDED'].value_counts(dropna=False)
