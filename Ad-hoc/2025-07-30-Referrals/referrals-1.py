...
#----------------------------------Bring in referral data

referrals = pd.read_excel("s3://alpha-omppg/data-central/PR_referrals/referrals_July2024_June2025.xlsx")
...
sum(referrals.duplicated('REVIEW_ID',keep=False)) # 0 duplicate Review IDs
sum(referrals.duplicated(['REVIEW_ID','ACTUAL'],keep=False)) # 0 duplicate Review IDs
referrals[referrals.duplicated(['REVIEW_ID','ACTUAL'],keep=False)]

sum(referrals.duplicated('PRISON_NUMBER',keep=False)) # 0
...
# Function to piece the NOMIS datases together
...
"""
matched.shape # 6764,6718, there are multiple mathces
...
"""
sum(matched.duplicated('REVIEW_ID',keep=False)) #4712, 5339
