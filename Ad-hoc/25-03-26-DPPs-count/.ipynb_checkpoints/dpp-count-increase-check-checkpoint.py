...
dppJun24[~dppJun24['NOMIS_ID'].isin(Mar24['NOMIS_ID'])]

dppJun24[dppJun24['ISP_STATUS']=='Unreleased IPP'] # 35
dppJun24[dppJun24['ISP_STATUS']=='Recalled IPP'] # 35

dppJun24.to_excel('DPPs.xlsx')
Mar24['LATEST_RELEASE_DATE']
...
add_pop = pd.read_sas(f"s3://alpha-omppg/isp-population/final/isp_pop_2024q1.sas7bdat", encoding='latin1')
add_pop.columns = add_pop.columns.str.upper()
add_pop.loc[add_pop['WHOLE_LIFE'].isin([True,'True']),'TARIFF_EXPIRY_DATE'] = pd.Timestamp.max.normalize()
add_pop = add_pop.drop(columns=['LATEST_RELEASE_DATE','EFFECTIVE_TED'],errors='ignore')       

pop = pd.concat([pop,add_pop],axis=0)
...
cols = ['NOMIS_ID','SURNAME','ISP_STATUS','EXTRACTDATE','DOS','CUSTODY_TYPE_DESCRIPTION','DATEOFBIRTH','PPUD_STATUS','OFFENCE','INDEX_OFFENCE_DESCRIPTION','PRISONNAME','CRO_NO','FORENAME']
dpp[cols].to_excel("DPPs.xlsx", index=False)
