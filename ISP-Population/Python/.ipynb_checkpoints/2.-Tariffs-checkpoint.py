...
dateColsToChange = ['LATEST_RELEASE_DATE','INDEX_OFFENCE_DATE']
...
for column in dateColsToChange:
    ispPPUD[column] = pd.to_datetime(ispPPUD[column], dayfirst = True)
...
ispPPUD_Matched.info()
ispPPUD_Matched.shape 

ispPPUD_Matched.loc[ispPPUD_Matched['WHOLE_LIFE'] == True,'TARIFF_EXPIRY_DATE'] = pd.Timestamp.max.normalize()

ispPPUD_Matched[ispPPUD_Matched['WHOLE_LIFE'] == True]['TARIFF_EXPIRY_DATE']
...
ispTariffs = duckdb.sql(query2).df()

# subset
ispTariffs = ispTariffs[(ispTariffs['SENTENCESTATUS'].isin(['(5) IPP','(6) Life'])) | 
                        (pd.notna(ispTariffs['CUSTODY_TYPE_DESCRIPTION'])) | 
                        (pd.notna(ispTariffs['TARIFF_EXPIRY_DATE']))]
ispTariffs.info()
ispTariffs.shape # 11389
...
ispTOffences = duckdb.sql(query3).df()

ispTOffences.shape

    # drop some columns
    
ispTOffences2 = ispTOffences2.drop(['MATCH','SURNAME_PPUD', 'DOB_PPUD', 'INIT_PPUD','PN_TRIM', 'PN_START', 'PN_END', 'NOMS_ID_PPUD', 'NOMS_TRIM', 'NOMS_START', 'NOMS_END', 'OFFENCE_UPPER'],axis = 1)
...
ispTOffences.loc[notIPP,'TARIFF_EXPIRY_DATE'] = np.nan
...
ispTOffences['TARIFFS_SERVED'] = np.where((ispTOffences['TARIFF_MONTHS'].isnull()) | (ispTOffences['TARIFF_EXPIRY_DATE'] == ispTOffences['DOS']),
                                         np.nan,
...
next_day = pd.Timestamp(2023,12,31)+ np.timedelta64(1,'D')

ispTOffences['TARIFF_IN_QUARTER'] = np.where((ispTOffences['TARIFF_EXPIRY_DATE'].dt.quarter == next_day.quarter) & 
                                             (ispTOffences['TARIFF_EXPIRY_DATE'].dt.year == next_day.year),
                                             ispTOffences['TARIFF_EXPIRY_DATE'], pd.NaT)
...
check1 = pd.read_sas("s3://alpha-omppg/ISP Population/Final Datasets/isp_pop_2024q1.sas7bdat", encoding='latin1')

check1['tariff'].value_counts(dropna=False).sort_index()
ispTOffences2['TARIFF'].value_counts(dropna = False).sort_index()

check1['OVERTARIFF_MONTHS'].value_counts(dropna=False).sort_index().head()
ispTOffences['OVERTARIFF_MONTHS'].value_counts(dropna = False).sort_index().head()

check1[~check1['tariff_in_quarter'].isna()]['tariff_in_quarter'].count()
ispTOffences[~ispTOffences['TARIFF_IN_QUARTER'].isna()]['TARIFF_IN_QUARTER'].count()

check1['TARIFF_YEARS'].value_counts(dropna=False)
ispTOffences['TARIFF_YEARS'].value_counts(dropna = False) 

ispTOffences[tpast1]['TARIFF_EXPIRY_DATE']
