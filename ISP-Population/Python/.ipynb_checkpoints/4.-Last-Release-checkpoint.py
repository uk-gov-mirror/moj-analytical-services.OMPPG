...
query2 = """SELECT DISTINCT a.*,                                    
            ...
            FROM ispFirstRel AS a LEFT JOIN isp_releases_2023q4 AS b
...
ispRelMatched = duckdb.sql(query2).df()
# ispRelMatched.info()
...
ispLasttRel =ispRelMatched.drop_duplicates(subset='NOMIS_ID', keep ='first').copy()
ispLasttRel.shape

#---------------------------------- drop some variables

ispLasttRel = ispLasttRel.drop(['MATCH', 'SURNAME_PPUD', 'DOB_PPUD', 'INIT_PPUD', 'PN2', 'PN_TRIM','PN_START', 
