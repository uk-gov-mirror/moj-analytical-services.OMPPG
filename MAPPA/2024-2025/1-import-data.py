...
# change hyphen to zeros
mappa_data = mappa_data.replace({'-':0})
...
# ------------
# Previously, we combined caution and conviction for Cat 1s subject to notification requirements
# code used was mappa_data['CAT1CAUTCON'] = mappa_data['CAT1L1CAUTCON'] + mappa_data['CAT1L2CAUTCON'] + mappa_data['CAT1L3CAUTCON']
# From 2025, we split these into caution and conviction
#------------

mappa_data['CAT1CAUT'] = mappa_data['CAT1L1CAUT'] + mappa_data['CAT1L2CAUT'] + mappa_data['CAT1L3CAUT']

mappa_data['CAT1CON'] = mappa_data['CAT1L1CON'] + mappa_data['CAT1L2CON'] + mappa_data['CAT1L3CON']
...
"""
pop_link = "https://www.ons.gov.uk/file?uri=/peoplepopulationandcommunity/populationandmigration/populationestimates/datasets/estimatesofthepopulationforenglandandwales/mid20242023localauthorityboundaries/mye24tablesew.xlsx"
...
ons_pop_data['10+'].sum() == pop_data[pop_data['Name'] == 'ENGLAND AND WALES']['10+'].values[0] # 55011163,54117598
...
mappa_data['10+'].sum() # must match the totals above, we use this figure to divide for England and wales in table 3
