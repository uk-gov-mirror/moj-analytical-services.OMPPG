...
# key_dtype = {col:'string' for col in pop.columns}
# pop = pd.read_excel(f"s3://alpha-omppg/Mental Health/{year}/Raw Data/Population.xlsx", dtype = key_dtype)
...
priority_move =['Abscond','Escape','Disposal By Court'] + list(set(moves['MOVE_SUB_TYPE_DESCRIPTION']) - set(['Abscond','Escape','Disposal By Court']))
...
moves = moves.sort_values(by=['FILE_REFERENCE','ACTUAL_DATE','AUTHORISATION_DATE'])
...
# moves2[moves2.duplicated(subset='FILE_REFERENCE',keep=False)].sort_values(['FILE_REFERENCE','ACTUAL_DATE']).head(10)
    ...
    # Aditional fields to identify Prior Statuses
moves2['PRIOR_ESTA'] = moves3['FROM_ESTABLISHMENT_DESCRIPTION']
