...
#--------------- Import discharges and disposals based on ACTUAL DATE (hitherto, it's been based on authorization date)

disch_and_disp = pd.read_excel(f"s3://alpha-omppg/Mental Health/{year}/Raw Data/Discharges_and_Disposals_Actual_Date_{year}.xls")

disch_and_disp = disch_and_disp.replace("–","-", regex = True) # replace long dashes

    # check datetime types
    
disch_and_disp.info() # 1806, all good
...
disch_and_disp.head() # 1806

#---------------Remove duplicates on both authorization date and Actual date

disch_and_disp[disch_and_disp.duplicated(subset=['FILE_REFERENCE','AUTHORISATION_DATE'],keep = False)].sort_values(['FILE_REFERENCE','AUTHORISATION_DATE']).head(10) # none

len(disch_and_disp) # 1806
...
disch_and_disp = disch_and_disp.drop(index=[313,1032,1060]) # prefer cd to release, prefer movement from hospital
...
disch_and_disp_2[~disch_and_disp_2['AUTHORISATION_DATE'].isna()].groupby(['DISPDISCH_TYPE']).size().reset_index(name='count')
...
disch_and_disp_2.to_parquet(f"s3://alpha-omppg/Mental Health/2023/Parquet Data/dispdisch__prepared_actual_date{year}.parquet")
    ...
    # Why using actual varies massively for (a) remain in hospital and (b) disposal not into community
        # from using date authorization date
        
disch_and_disp_2['AUTHORISATION_DATE'].dt.year.value_counts(dropna=False) # Too many NaNs may explain the difference

disch_and_disp_2[disch_and_disp_2['AUTHORISATION_DATE'].isna()].groupby(['DISPDISCH_TYPE']).size().reset_index(name='count') # greately affects these two categories (90 each)

disch_and_disp_2[disch_and_disp_2['AUTHORISATION_DATE'].isna()].head()
