...
admis = pd.read_excel(f"s3://alpha-omppg/Mental-Health/{year}/raw-data/Admissions_{year}.xls")
recs = pd.read_excel(f"s3://alpha-omppg/Mental-Health/{year}/raw-data/Recalls_{year}.xls")

admis_and_recs = pd.concat([admis,recs], axis=0)
len(admis_and_recs) # 1568
...
admis_and_recs[admis_and_recs.duplicated(subset=['FILE_REFERENCE','ACTUAL_DATE'],keep = False)].sort_values(['FILE_REFERENCE','ACTUAL_DATE'])

# admis_and_recs.loc[[1037,962]]
admis_and_recs = admis_and_recs.drop(index=[1037,962])

len(admis_and_recs) # 1566, 1748

#---------------Remove Test cases
    # Check 'test' cases and remove
admis_and_recs[admis_and_recs['FAMILY_NAME'].str.contains('Test|Lumen',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']]
