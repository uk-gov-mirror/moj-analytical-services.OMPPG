...
disch_and_disp = pd.read_excel(f"s3://alpha-omppg/Mental-Health/{year}/raw-data/RP-disposals-and-discharges-{year}.xls")
...
show_data(disch_and_disp[disch_and_disp.duplicated(subset=['FILE_REFERENCE','AUTHORISATION_DATE'],keep = False)].sort_values(['FILE_REFERENCE','AUTHORISATION_DATE']))# none

disch_and_disp = disch_and_disp.drop(index=[2,8,5,0,17,780])

len(disch_and_disp) # 1773, 1602
...
disch_and_disp = disch_and_disp.drop(index=[1034,1090]) # some duplicates and incorrect entries

len(disch_and_disp) # 1771

#---------------Remove Test cases
   
disch_and_disp[disch_and_disp['FAMILY_NAME'].str.contains('Test|Lumen',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] #0

disch_and_disp[disch_and_disp['FIRST_NAMES'].str.contains('Test|Lumen',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']]
...
disch_and_disp_2.groupby(['DISPDISCH_TYPE']).size().reset_index(name='count').to_excel("Tables/Table 8.xlsx", index=False)
...
disch_and_disp_2['CRO_PNC'] = disch_and_disp_2['CRO_PNC'].astype(str)

disch_and_disp_2.to_parquet(f"s3://alpha-omppg/Mental-Health/{year}/output/dispdisch-prepared-{year}.parquet")

disch_and_disp_2.to_excel("Disch_and_Disp2.xlsx")
