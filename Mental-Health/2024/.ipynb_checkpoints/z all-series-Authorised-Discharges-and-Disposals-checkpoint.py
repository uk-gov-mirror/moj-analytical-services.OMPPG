...
#disch_and_disp = disch_and_disp[~disch_and_disp['AUTHORISATION_DATE'].isna()]

len(disch_and_disp) # 47735
...
#---------------TODORemove duplicates on both authorization date and Actual date

sum(disch_and_disp.duplicated(subset=['FILE_REFERENCE','AUTHORISATION_DATE'],keep = False)) # 149
sum(disch_and_disp.duplicated(subset=['FILE_REFERENCE','ACTUAL_DATE'],keep = False)) # 169
...
# where duplicate authorization date, keep those with actual dates
disch_and_disp = disch_and_disp.sort_values(['FILE_REFERENCE','AUTHORISATION_DATE','ACTUAL_DATE'],ascending=[True,True,False])

len(disch_and_disp) # 47735

disch_and_disp[disch_and_disp.duplicated(subset=['FILE_REFERENCE','ACTUAL_DATE'],keep = False)].sort_values(['FILE_REFERENCE','ACTUAL_DATE']).head(10) # 6 rows

# disch_and_disp = disch_and_disp.drop(index=[293,959]) # some duplicates and incorrect entries
...
disch_and_disp['DISPDISCH_TYPE'].value_counts(dropna=False)
...
show_data(
    disch_and_disp[disch_and_disp['DISPDISCH_TYPE'] == 'check'].groupby(['MOVE_SUB_TYPE_DESCRIPTION','MOVE_SUB_SUB_TYPE_DESCRIPTION','DISPDISCH_TYPE']).size().reset_index(name='count')
)# check any non-matches

show_data(
    disch_and_disp[(disch_and_disp['DISPDISCH_TYPE'] == 'check') & (disch_and_disp['AUTHORISATION_DATE'].dt.year >= 2014)].groupby(['MOVE_SUB_TYPE_DESCRIPTION','MOVE_SUB_SUB_TYPE_DESCRIPTION','DISPDISCH_TYPE']).size().reset_index(name='count')
)

disch_and_disp['YEAR_AUTHORIZED'] = disch_and_disp['AUTHORISATION_DATE'].dt.year

show_data(
    disch_and_disp.pivot_table(index=['MOVE_SUB_TYPE_DESCRIPTION','MOVE_SUB_SUB_TYPE_DESCRIPTION','DISPDISCH_TYPE'],
                           columns='YEAR_AUTHORIZED',
                          fill_value=0,
                          aggfunc='size',
