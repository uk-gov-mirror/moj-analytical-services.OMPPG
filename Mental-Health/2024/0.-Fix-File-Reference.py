""" 
GOAL: ADD FILE REFERENCE FROM OFFENCE DATA TO THE POULATION DATA EMMA SENT ME.
...
offences = pd.read_excel(f"s3://alpha-omppg/Mental-Health/2024/raw-data/offences_{year}.xls")
offences = offences.replace("–","-",regex = True)

offences.head()
len(offences) # 13513

sum(offences.duplicated(['FILE_REFERENCE'],keep=False)) # 8673

column_order = ['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES','AUTHORITY_FOR_DETENTION_DESCRIPTION',
               'OFFENCE_DESCRIPTION','COURT_OFFENCE_TEXT','DATE_OF_HOSPITAL_ORDER']

offences = offences[column_order + [col for col in offences.columns if col not in column_order]]

show_data(offences[offences.duplicated(['FILE_REFERENCE'],keep=False)].head(50))

    # keep one unique file reference-prison number prison combo
offences = offences.drop_duplicates(subset=['FILE_REFERENCE'])
#len(offences) # 30856

pop = pd.read_excel(f"s3://alpha-omppg/Mental Health/{year}/Raw Data/population_initial.xlsx")
