...
admis_and_recs = pd.read_excel(f"s3://alpha-omppg/Mental-Health/{year}/raw-data/RP-admissions-and-recalls-{year}.xls")

len(admis_and_recs) # 1743
...
admis_and_recs[admis_and_recs.duplicated(subset=['FILE_REFERENCE','ACTUAL_DATE'],keep = False)].sort_values(['FILE_REFERENCE','ACTUAL_DATE']) # 2 dups

# admis_and_recs.loc[[1037,962]]
admis_and_recs = admis_and_recs.drop(index=[409,1645])

len(admis_and_recs) # 1741, 1748

#---------------Remove Test cases
    # Check 'test' cases and remove
admis_and_recs[admis_and_recs['FAMILY_NAME'].str.contains('Test|Lumen',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] #0

admis_and_recs[admis_and_recs['FIRST_NAMES'].str.contains('Test|Lumen',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']]
...
len(admis_and_recs) # 1741, 1747

    # Check 'case' cases and remove
admis_and_recs[admis_and_recs['FAMILY_NAME'].str.contains('Case',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] # 1
...
detAuth_dict = dict(
    zip(detAuth['AUTHORITY_FOR_DETENTION_DESCRIPT'],detAuth['DETAUTH_PUB'])
) # creates a dictionary of DAs and their pub values

# for key,value in detAuth_dict.items():
    # print(f"{key} = {value}")
...
admis_and_recs['AUTHORITY_FOR_DETENTION_DESCRIPTION'].value_counts(dropna=False)
admis_and_recs = admis_and_recs[~(admis_and_recs['AUTHORITY_FOR_DETENTION_DESCRIPTION'] == 'Not Applicable')]

len(admis_and_recs) # 1741, 1746
...
admis_and_recs['HOSP_TYPE'].value_counts(dropna=False)
...
admis_and_recs_2.groupby(['ADMISSION_CATEGORY'],observed=False).size().reset_index(name='count').to_excel("Tables/Table 7.xlsx", index=False)
...
admis_and_recs_2.to_parquet(f"s3://alpha-omppg/Mental-Health/{year}/output/admissions-recalls.parquet")

admis_and_recs_2.to_excel("admission-recalls-2024.xlsx",index=False)
