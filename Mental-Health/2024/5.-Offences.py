...
offences = pd.read_excel(f"s3://alpha-omppg/Mental-Health/{year}/raw-data/offences_{year}.xls")
...
# offences['NOMS_ID'] = offences['NOMS_ID'].astype(str)
...
query = """SELECT a.*, 
            ...
            ON  (
                    ...
                    /*OR (a.PRISON_NUMBER = b.PRISON_NUMBER AND a.PRISON_NUMBER IS NOT NULL) */
                ) AND
...
len(non_missing_offence_3)
    # Resolve missing offence cases
missing_offence_3 = pop_off3[pop_off3['OFFENCE_DESCRIPTION'].isna()]
len(missing_offence_3) # 8


# ----------------- Put together sorted missing offences

resolved_missing_offence = pd.concat([non_missing_offence_1,non_missing_offence_2,non_missing_offence_3,missing_offence_3],ignore_index=True)
...
len(resolved_missing_offence) # 13109, 12835

resolved_missing_offence.head()

resolved_missing_offence[resolved_missing_offence['offencegrp_new'].isna()] # only 9
...
len(unique_offence_description) # 12297, 12077
...
len(unique_offence_group) # 10948, 10785
...
unique_offence_group.to_parquet(f"s3://alpha-omppg/Mental-Health/{year}/output/offence_groups_prepared_{year}.parquet")

most_ser.to_parquet(f"s3://alpha-omppg/Mental-Health/{year}/output/population_with_one_offence_prepared_{year}.parquet")

most_ser.to_excel("population_with_one_offence_prepared_2024.xlsx",index=False)
...
pd.crosstab(most_ser['DA_CUSTODY_TYPE_DESCRIPTION'],most_ser['STATUS'], margins = True, margins_name = 'Total').to_excel("Tables/Table_1_CusType.xlsx")