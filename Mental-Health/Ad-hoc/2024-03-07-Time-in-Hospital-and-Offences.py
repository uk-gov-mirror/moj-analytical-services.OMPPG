...
#--------------- Import othe population dataset and replace long dash with normal dash

fl_off = pd.read_excel("s3://alpha-omppg/Mental Health/2023/Raw Data/Offence_all.xls")
fl_off = fl_off.replace("–","-",regex=True)
...
recalls = recalls.replace("–","-",regex=True)
...
# pop_Admin_Rec.to_parquet("pop_Admin_Rec.parquet")
...
rlv_Auths['TIME_CATEGORY'].value_counts(dropna=False).sort_index()
