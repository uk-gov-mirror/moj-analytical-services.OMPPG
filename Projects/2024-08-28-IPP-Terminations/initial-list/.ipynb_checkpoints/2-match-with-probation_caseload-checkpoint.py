...
wanted.loc[(wanted['AUTO_TERMINATION_DATE'] <= pd.Timestamp(2024,12,10)) & (wanted['CURRENT_STATUS'] == 'Released'),'AUTO_TERMINATION_10DEC2024'] = 'Immediate termination'
wanted.loc[(wanted['AUTO_TERMINATION_DATE'] > pd.Timestamp(2024,12,10)) & (wanted['CURRENT_STATUS'] == 'Released'),'AUTO_TERMINATION_10DEC2024'] = wanted['AUTO_TERMINATION_DATE'].dt.strftime('%B, %Y')

# Active Reviews

ipp_active_gpp = pd.read_excel('raw-data/ipp-active-GPP.xls')
...
ipp_term_reviews = pd.read_excel('raw-data/ipp-termination-reviews.xls')
