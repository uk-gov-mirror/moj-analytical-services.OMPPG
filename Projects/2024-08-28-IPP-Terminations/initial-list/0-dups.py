...
# len(data2)
# data2[data2.duplicated('ID',keep=False)][['ID','FILE_REFERENCE','FAMILY_NAME','DOS','STATUS_DESCRIPTION','OLD_STATUS']]
...
# all_ipp_dpp['MISSING_NOMS'].value_counts() # 235 missing NOMS ID
...
#dups[dups['COMMON_ID'].isna()][retain + ['TEMP_FN','DOB']]# 3 cases
...
all_ipp_dpp.loc[2370,'COMMON_ID'] = 'A7601AE'

#all_ipp_dpp[all_ipp_dpp['COMMON_ID'].isna()][retain + [] + [col for col in all_ipp_dpp.columns if col not in retain]]

#len(all_ipp_dpp[all_ipp_dpp['COMMON_ID'].isna()]) # 218
