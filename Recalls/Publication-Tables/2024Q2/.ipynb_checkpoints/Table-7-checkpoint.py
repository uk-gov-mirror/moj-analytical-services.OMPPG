...
ual = pd.concat( [ual3,ual4,ual5,ual1,ual2], ignore_index=True)
len(ual) # 12236, 11900
ual.head()
ual.info()
...
with pd.ExcelWriter(output_work_book,engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
    table_7_df.T.reset_index().T.to_excel(writer,startrow = 4, sheet_name='Table_7',index=False,header=False)
