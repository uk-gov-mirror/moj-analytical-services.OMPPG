...
reasons = pd.concat([reas3,reas4,reas5,reas1,reas2], ignore_index=True)
len(reasons) # 74661,68905
reasons.head()
reasons.info()
...
with pd.ExcelWriter(output_work_book,engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
    table_10_df.T.reset_index().T.to_excel(writer,startrow = 4, sheet_name='Table_10',index=False,header=False)