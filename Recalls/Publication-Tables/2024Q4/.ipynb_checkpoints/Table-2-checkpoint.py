...
rec1 = pd.read_sas('s3://alpha-omppg/Recalls/final_data/recalls/all/recalls_final_2023q2.sas7bdat',encoding='latin1')
...
with pd.ExcelWriter(output_work_book,engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
    table_2_df.T.reset_index().T.to_excel(writer,startrow = 4, sheet_name='Table_2',index=False, header = False)
