...
rec3 = pd.read_parquet('s3://alpha-omppg/Recalls/final_data/recalls/all/recalls_final_2025q1.parquet')
...
recalls = pd.concat([rec4,rec5,rec1,rec2,rec3], ignore_index=True)
len(recalls) # 47673, 44725, 41354, 38193,35235
recalls.head()
...
addSentenceMask = (
    (recalls['QUARTER'] == 'Jan to Mar 2025') & 
    (recalls['RECALL_TYPE_DESCRIPTION'].isin(trbHdcMask))
)

sum(addSentenceMask) # 349

oraMask2 = addSentenceMask & (recalls['PART_TOTAL_IN_DAYS'] < 365)
sum(oraMask2) # 89

recalls[addSentenceMask]['PART_TOTAL_IN_DAYS'].value_counts(dropna=False).sort_index()
recalls[addSentenceMask]['PART_TOTAL_IN_DAYS'].min() # 113
recalls[addSentenceMask]['PART_TOTAL_IN_DAYS'].max() # 5170
recalls[addSentenceMask]['SENTENCE'].unique()

"""

recalls.loc[oraMask2,'SENTENCE'] = 'Determinate less than 12 months'
