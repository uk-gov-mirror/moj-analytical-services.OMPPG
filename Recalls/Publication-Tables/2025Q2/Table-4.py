...
recalls['UAL_FLAG'].unique()
recalls.info() # UAL_FLAG is now boolean
...
with pd.ExcelWriter(output_work_book,engine='openpyxl', mode ='a', if_sheet_exists='overlay') as writer:
    table_4_df.T.reset_index().T.to_excel(writer,startrow = 4, sheet_name='Table_5_Q_4',index=False,header = False)
