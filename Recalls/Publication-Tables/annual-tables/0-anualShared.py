...
def ethnicity(df):
    ...
    df['ETHNICITY'] = np.select(table_12_conditions, choices, default=df['ETHNICITY_DESCRIPTION'])

ethnicity_vals = ['Asian or Asian British', 'Black or Black British','Mixed','White', 'Other ethnic group', 'Not stated','Unknown']
...
def format_quarter(period):
    ...
    return f"{quarter_mapping[f'Q{quarter_str}']} {quarter}"
...
ftr_std_values = ['All recalls', 'Fixed term - 14 days', 'Fixed term - 28 days','Standard - determinate sentence','Standard - indeterminate sentence']


# probation_region values = recalls['NPS_CRC_NAME'].unique()

probation_region = ['Bedforshire, Northamptonshire, Cambridgeshire & Hertfordshire',
       'Bristol, Gloucestershire, Somerset & Wiltshire',
       'Cheshire & Greater Manchester', 'Cumbria & Lancashire',
       'Derbyshire, Leicestershire, Nottinghamshire & Rutland',
       'Dorset, Devon & Cornwall', 'Durham Tees Valley', 'East Midlands',
       'East of England', 'Essex', 'Greater Manchester',
       'Hampshire & Isle of Wight',
       'Humberside, Lincolnshire & North Yorkshire', 'Kent Surrey Sussex',
       'Kent, Surrey & Sussex', 'London', 'Merseyside', 'Midlands',
       'National Security Division', 'Norfolk & Suffolk', 'North East',
       'North West', 'Northumbria', 'South Central', 'South East & Eastern',
       'South West', 'South West & South Central', 'South Yorkshire',
       'Staffordshire & West Midlands', 'Thames Valley', 'Unassigned', 'Wales',
       'Warwickshire & West Mercia', 'West Midlands', 'West Yorkshire',
       'Yorkshire and The Humber']


supervising_body = ['All supervising bodies', 'National probation','National security division', 'Community rehabilitation companies','Unknown']
...
def table_2_func(df, sex_values, sentence_values, hdc_values): # parameters are in Shared.py
    ...
    # [ {'sex':val, 'Sentence type':val,'Recall category':val, 'year': val,'values': val}, ...]
    ...
    # For each combination of sex and sentence, count entries for each year - this is the top line with 'All recalls'
    # For each combination of sex and sentence, filter for true HDC type and count entries for each year - this is subsequent line after 'All recalls' line
    ...
    for sex in sex_values:
        for sentence in sentence_values:
            ...
            for year in years:
                temp_date_df = temp_df[temp_df['YEAR'] == year]
    ...
    # Sex, Sentence type, Recall category, year and values
...
def table_3_func(df, supervising_body,probation_region):
    
    summary_list = []
    
    for supBody in supervising_body:
        if supBody == 'All supervising bodies':
            temp_df = df
            ...
            temp_df = df[df['SUP_BODY'] == supBody]
        ...
        for year in years:
            ...
            for region in probation_region:  # Skip 'All recalls' in this loop
                type_value = temp_date_df[temp_date_df['NPS_CRC_NAME'] == region]['LICENCE_REVOKE_DATE'].count()
                summary_list.append({
                    'Supervising body': supBody,
                    'Region': region,
                    'year': year,
    ...
    for sex in sex_values:
        if sex == 'Male and female':
            for sentence in sentence_values:
                ...
                for year in years:
                    temp_date_df = temp_df[temp_df['YEAR'] == year]              
                    year_value = temp_date_df['LICENCE_REVOKE_DATE'].count()
                    summary_list.append({
                            'Sex': sex,
                            'Sentence type': sentence,
