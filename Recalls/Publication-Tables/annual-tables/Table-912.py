...
recalls.pivot_table(index=['ETHNICITY_DESCRIPTION','ETHNICITY'],columns='COMMON',aggfunc='size')
# create a list of the unique recall['QUARTER'] values ---------------

recalls['ETHNICITY'].unique()
years
...
table_12_df = table_12_df[['Sex', 'Sentence type', 'Ethnicity'] + years]
...
with pd.ExcelWriter(output_work_book,engine='openpyxl', mode='a',if_sheet_exists='overlay') as writer:
    table_12_df.T.reset_index().T.to_excel(writer,startrow = 4, sheet_name='Table_5_Q_12',index=False,header=False)