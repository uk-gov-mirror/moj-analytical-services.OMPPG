...
year = 2023 
quarter = 4 
# rec_ppud_folder = "s3://alpha-omppg/Recalls/"
...
recallsPPUD.info() # 9695

    # Convert columns that should be datetime to datetime
dateColsToChange =['LICENCE_REVOKE_DATE','RTC_DATE','TARIFF_EXPIRY_DATE','ISSUE_REVOCATION_TARGET','PB_DECISION_AFTER_BREACH_ACTUAL','REPORT_RECD_BY_UNIT_ACTUAL','REPORT_RECD_BY_UNIT_TARGET','RTC_TARGET']
...
check1.head()
check1.shape #279 cases, mostly missing data represented as '00/01/1900 00:00:00'

    # Make two corrections to dates
for column in dateColsToChange:
    recallsPPUD[column] = recallsPPUD[column].astype(str).str.replace("00/01/1900 00:00:00", "")
    recallsPPUD[column] = recallsPPUD[column].astype(str).str.replace("14/09/2997", "14/09/2007") 
...
for column in dateColsToChange:
    recallsPPUD[column] = pd.to_datetime(recallsPPUD[column], dayfirst = True)
...
recallsPPUD[Test_Case_Mask][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] # 3 cases

recallsPPUD2 = recallsPPUD[~Test_Case_Mask]
recallsPPUD2.shape #9692
...
recalls_nodup.to_pickle(f"isp_recalls_{year}q{quarter}_step1.pkl")
