...
disch20132015 = pd.read_csv(f's3://alpha-omppg/ISP Releases/NOMIS/discharges20132015.csv')
disch20132015.info()
disch20132015.head()
...
# disch20132015 = disch20132015.convert_dtypes() # strings, datetime,int,float, no object type
...
releases = pd.read_excel(f's3://alpha-omppg/ISP Releases/PPUD/PPUD_Releases_{year}Q{quarter}.xls')
releases_2010 = pd.read_excel(f's3://alpha-omppg/ISP Releases/PPUD/Releases 2010.xls')
releases_2011 = pd.read_excel(f's3://alpha-omppg/ISP Releases/PPUD/Releases 2011.xls')
releases_2012 = pd.read_excel(f's3://alpha-omppg/ISP Releases/PPUD/Releases 2012.xls')
...
# releases = releases.convert_dtypes() # strings, datetime,int,float, no object type

# for col in releases.select_dtypes(include='object').columns:
    # releases[col] = releases[col].astype('string')
...
releases[releases['FAMILY_NAME'].str.contains('Test',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']]

releases[releases['FIRST_NAMES'].str.contains('Test',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']]

Test_Case_Mask =  (   (releases['FAMILY_NAME'].str.contains('Test',case = False,na = False)) |
                      (releases['FIRST_NAMES'].str.contains('Test',case = False,na = False))
...
releases.shape  # 20192

    # Check 'case' cases and remove
releases[releases['FAMILY_NAME'].str.contains('Case',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] # 0

releases[releases['FIRST_NAMES'].str.contains('Case',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] # 0
...
#---------------------------------- Release date must not be after dos
...
releases_matched[releases_matched.duplicated(subset='UNIQUEREF',keep=False)][['NOMS_ID','FAMILY_NAME','UNIQUEREF','MATCH','DATEDIF','RELEASE_DATE','DATEDIS']]

# releases[releases.duplicated(subset=['FILE_REFERENCE','RELEASE_DATE'],keep=False)][['NOMS_ID','FAMILY_NAME','FILE_REFERENCE','RELEASE_DATE']]
   
    # Correct cases where DATEDIF is smaller but MATCH is lower
...
Releases_final2.to_pickle(f"isp_releases_{year}q{quarter}_step1.pkl")
