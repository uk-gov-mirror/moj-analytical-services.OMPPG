...
isp_releases_final = prepareMatch.prepareMatch(isp_releases_final)
recalls_nodup = prepareMatch.prepareMatch(recalls_nodup)

    # drop the length variables
isp_releases_final.drop(['PN_LENGTH','NOMS_LENGTH'],axis=1,inplace = True)
...
release_to_recall2 = duckdb.sql(query).df()
release_to_recall2.shape # 16002
...
release_to_recall3 = duckdb.sql(query).df()
release_to_recall3.shape # 9916
...
isp_recalls_final.shape #9891

isp_recalls_final.head()

#---------------------------------- Save to Amazon

isp_recalls_final.to_parquet(f"s3://alpha-omppg/Recalls/Final Data/Parquet/isp_recalls_{year}q{quarter}.parquet",index=False)
