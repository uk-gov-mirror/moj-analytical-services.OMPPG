...
recall_to_release.shape # 17746, 17194, 16668,16315,15905
...
recall_to_release.shape # 17740, 17188,16663,16310, 15900
...
recall_to_release2.shape # 18616, 18050,17512,17139, 16707
...
isp_releases_final.shape # 17740, 17188,16663,16310,15900
...
# --------------------------- RECALLED_WITHIN_X_MONTHS
'''
Logic: (1) OF THOSE who have spent AT LEAST 3 months in the community to the end of the current period, 
           which ones got recalled within 3 months?.
       (2) OF THOSE who have spent AT LEAST 6 months in the community to the end of the current period, 
           which ones got recalled within 6 months?.
           
Caveat: This is NOT determining who got recalled with 3 months of all those released up to the end of the current period.
        Notice that some could have been released and subsequently recalled within, say, 5 days to the end of the current
        period; however, these would not have had full three months in the community, so we are not interested in them.
        
Future dev: Is this useful? What is the alternative? 

''' 

isp_releases_final['LATEST_QUARTER'] = pd.Timestamp(year, quarter * 3 - 2, 1) - pd.Timedelta(days=1) # end of previous quarter

isp_releases_final.head()

relAtleast3MthsAgo = (isp_releases_final['RELEASE_DATE'] <= isp_releases_final['LATEST_QUARTER'])
relAtleast6MthsAgo = isp_releases_final['RELEASE_DATE'] <= (isp_releases_final['LATEST_QUARTER'] - pd.DateOffset(months = 3))
relAtleast12MthsAgo = isp_releases_final['RELEASE_DATE'] <= (isp_releases_final['LATEST_QUARTER'] - pd.DateOffset(months = 9))

isp_releases_final['RECALLED_3_MONTHS'] = np.where(relAtleast3MthsAgo, 
                                                   np.where(isp_releases_final['MONTHS_UNTIL_RECALL'] < 3,100,0),
                                                   np.nan)

isp_releases_final['RECALLED_6_MONTHS'] = np.where(relAtleast6MthsAgo, 
                                                   np.where(isp_releases_final['MONTHS_UNTIL_RECALL'] < 6,100,0),
                                                   np.nan)

isp_releases_final['RECALLED_12_MONTHS'] = np.where(relAtleast12MthsAgo, 
                                                    np.where(isp_releases_final['MONTHS_UNTIL_RECALL'] < 12,100,0),
                                                    np.nan)

#### FIRST OR SUBSEQUENT RELEASES?
