...
recalls.shape,extras.shape #((7157, 34), (7152, 42))
...
dateColsToChange =['LICENCE_REVOKE_DATE','RTC_DATE','ISSUE_REVOCATION_TARGET','PB_DECISION_AFTER_BREACH_ACTUAL','REPORT_RECD_BY_UNIT_ACTUAL','REPORT_RECD_BY_UNIT_TARGET','RTC_TARGET']
...
check1.head()
check1.shape #19 cases, mostly missing data represented as '00/01/1900 00:00:00'
check1

    # delete some stopped cases
recalls = recalls.drop(index=[4320,4876,5493,6231]) # stopped/deceased

    # Make two corrections to dates
for column in dateColsToChange:
    recalls.loc[recalls[column].astype(str).str.contains('LATER'), column] = np.nan
...
recalls2[recalls2['FAMILY_NAME'].str.contains('Case',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] # 0

recalls2[recalls2['FIRST_NAMES'].str.contains('Case',case = False,na = False)][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] # 0

    # Check 'digit' cases - these are normally good and shoulbe untouched
recalls2[recalls2['FAMILY_NAME'].str.contains(r'\d')][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] 
recalls2[recalls2['FIRST_NAMES'].str.contains(r'\d')][['FILE_REFERENCE','FAMILY_NAME','FIRST_NAMES']] 

#---------------------------------- Drop duplicates
recalls2.shape
recalls2 = recalls2.drop_duplicates()
...
recalls_pub.shape # 7153 no dups
...
recalls_pub[recalls_pub['RECALL_REASON_DESCRIPTIONS'].isna()][['FILE_REFERENCE', 'PRISON_NUMBER', 'FAMILY_NAME', 'LICENCE_REVOKE_DATE', 'RTC_DATE', 'RECALL_TYPE_DESCRIPTION']]

    # delete some other stopped cases
recalls_pub = recalls_pub.drop(index=[7152]) # cancelled, no rtc
...
# drop some variables
recalls_pub = recalls_pub.drop(columns=['EXTRAS_FAMILY_NAME','EXTRAS_FIRST_NAME','FR','PN'],)
...
recalls_unmatched_codes = duckdb.sql(query).df()
...
recalls_final.to_parquet(f"s3://alpha-omppg/Recalls/Final Data/Parquet/recalls_final_{year}q{quarter}.parquet",index=False)