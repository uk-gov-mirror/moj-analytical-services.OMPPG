...
recalls_final['NUMBER_OF_RECALL_REASONS'].dropna().sum()
...
reasons_final['YYQ'].head()

# Create a pivot table
pd.pivot_table(reasons_final, index=['GENDER', 'REASON_DESC'], columns=['YYQ'], aggfunc='size', fill_value=0, margins=True, margins_name='All')
...
reasons_final.to_parquet(f"s3://alpha-omppg/Recalls/Final Data/Parquet/recall_reasons_{year}q{quarter}.parquet",index=False)