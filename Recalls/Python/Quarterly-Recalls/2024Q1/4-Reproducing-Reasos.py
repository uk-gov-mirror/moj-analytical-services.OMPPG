...
recalls_final = pd.read_sas("s3://alpha-omppg/Recalls/final_data/recalls/recalls_final_2023q3.sas7bdat",encoding='latin1')

recalls_final.columns = recalls_final.columns.str.upper()
...
recalls_final['NUMBER_OF_RECALL_REASONS'].dropna().sum() #14626
...
pd.pivot_table(reasons_final, 
               ...
               columns=['YYQ'], aggfunc='size', fill_value=0, margins=True, margins_name='All')
...
reasons_final.to_parquet(f"s3://alpha-omppg/Recalls/final_data/recall_reasons/recall_reasons_{2023}q{3}.parquet",index=False)
