...
recalls_final['NUMBER_OF_RECALL_REASONS'].sum() # 19643, 18840, 19051, 14626
...
reasons.shape # # 19643, 18840, 19051, 14626
...
recallReason_format = {
    ...
    'HDC - Failed visit': 'Failed home visit',
    'HDC - Failed installation': 'HDC - Failed installation',
    'HDC - Equipment Tamper': 'Equipment Tamper',
    'EM - Equipment Tamper': 'Equipment Tamper',
    'Breach of Exclusion Zone': 'Breach of Exclusion Zone',
    'Travelling outside the UK':'Travelling outside the UK',
    'Missing': 'Unknown'
}

# Apply the mapping to 'REASON_DESC'
reasons['REASON_DESC_2'] = reasons['REASON_DESC'].map(recallReason_format).fillna('Other')
reasons.head()

reasons.pivot_table(index=['REASON_DESC_2','REASON_DESC'],columns='RETURN_BY',aggfunc='size')

reasons['REASON_DESC'] = reasons['REASON_DESC_2']
reasons = reasons.drop(columns='REASON_DESC_2')
...
reasons_final.shape # 19589, 18781, 18977, 14579
