...
rec5 = pd.read_parquet('s3://alpha-omppg/Recalls/final_data/recalls/all/recalls_final_2024q1.parquet')
rec6 = pd.read_parquet('s3://alpha-omppg/Recalls/final_data/recalls/all/recalls_final_2024q2.parquet')
rec7 = pd.read_parquet('s3://alpha-omppg/Recalls/final_data/recalls/all/recalls_final_2024q3.parquet')

# uppercase the headers
for df in [rec1,rec2,rec3,rec4,rec5,rec6,rec7]:
    df.columns = df.columns.str.upper()

# Concatenate all DataFrames into one------------------------------------------------------------------

recalls = pd.concat([rec1,rec2,rec3,rec4,rec5,rec6,rec7], ignore_index=True)
len(recalls)
recalls.head()
recalls.info()

del rec1,rec2,rec3,rec4,rec5,rec6,rec7
...
sentence_summary = recalls.groupby(['SENTENCE','FIXED', recalls['LICENCE_REVOKE_DATE'].dt.to_period('Q')]).size().reset_index(name='Total')

show(sentence_summary.pivot_table(index =['SENTENCE','FIXED'],columns = 'LICENCE_REVOKE_DATE',values = 'Total').reset_index(),
     buttons=["excelHtml5"])
