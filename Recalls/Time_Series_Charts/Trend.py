...
rec0 = pd.read_sas('s3://alpha-omppg/Recalls/final_data/recalls/recalls_final_2023q3.sas7bdat',encoding='latin1')
rec1 = pd.read_parquet('s3://alpha-omppg/Recalls/final_data/Parquet/recalls_final_2023q4.parquet')
rec2 = pd.read_parquet('s3://alpha-omppg/Recalls/final_data/Parquet/recalls_final_2024q1.parquet')

# uppercase the headers
for df in [rec0,rec1,rec2]:
    df.columns = df.columns.str.upper()

# Concatenate all DataFrames into one------------------------------------------------------------------

recalls = pd.concat([rec0,rec1,rec2], ignore_index=True)
len(recalls) # 21597
recalls.head()
recalls.info()
...
sentence_summary.pivot_table(index ='SENTENCE',columns = 'LICENCE_REVOKE_DATE',values = 'Total').reset_index()

# Tabulate - ORA

ora_summary = recalls.groupby(['ORA', recalls['LICENCE_REVOKE_DATE'].dt.to_period('Q')]).size().reset_index(name='Total')
ora_summary.pivot_table(index ='ORA',columns = 'LICENCE_REVOKE_DATE',values = 'Total').reset_index()

# Tabulate - HDC

hdc_summary = recalls.groupby(['HDC', recalls['LICENCE_REVOKE_DATE'].dt.to_period('Q')]).size().reset_index(name='Total')
hdc_summary.pivot_table(index ='HDC',columns = 'LICENCE_REVOKE_DATE',values = 'Total').reset_index()
...
std_1_summary.pivot_table(index ='FIXED',columns = 'LICENCE_REVOKE_DATE',values = 'Total').reset_index()
...
std_2_summary.pivot_table(index ='FIXED',columns = 'LICENCE_REVOKE_DATE',values = 'Total').reset_index()
